...
export function StatusBadges({ status }: StatusBadgesProps) {
  ...
  const getPipelineVariant = (state: string) => {
    switch (state) {
      case "running":
        return "default" // using "default" instead of "success" as it's a standard Badge variant
      case "stopped":
        return "secondary"
      default:
        return "outline"
  ...
  const getConnectionVariant = (state: string) => {
    switch (state) {
      case "up":
        return "default" // using "default" instead of "success" as it's a standard Badge variant
      case "down":
        return "destructive" // using "destructive" instead of "error" as it's a standard Badge variant
      default:
        return "outline"
    }
  }
